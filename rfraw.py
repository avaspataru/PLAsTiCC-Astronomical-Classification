    ...
    if mode==0:
        ...
        for obj_id, d in get_objects_by_id(filepath):
            ...
            if(count == 10000):
                ...
                if(batch_no==0):
                    finish.to_csv("preds_rfraw.csv", index = False, header = True)
                else:
                    with open("preds_rfraw.csv", 'a') as f:
        ...
        print " >>Clean up."
        preds = pd.read_csv("preds_rfraw.csv")
        preds['object_id']=preds['object_id'].apply(int)
	    #preds['object_id']=preds['object_id'].apply(int)
        print preds.shape
        print cc
        preds.to_csv('preds_rfraw2.csv', index=False)
