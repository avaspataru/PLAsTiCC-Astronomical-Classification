...
def my_predict(column_names,my_extra_data_list, my_intra_data_list, test_set_metadata_raw, extra_model, intra_model):
    ...
    if(len(my_intra_data_batch.index)>0):
        ...
        batch_intra_dataFrame = dropMagn(batch_intra_dataFrame)
        scaler = StandardScaler()
        #print batch_intra_dataFrame
...
def dropMagn(data):
    ddata = data.copy()
    ddata = ddata.drop('absmagn_0',axis=1)
    ddata = ddata.drop('absmagn_1',axis=1)
    ddata = ddata.drop('absmagn_2',axis=1)
    ddata = ddata.drop('absmagn_3',axis=1)
    ddata = ddata.drop('absmagn_4',axis=1)
    ddata = ddata.drop('absmagn_5',axis=1)
    return ddata
...
def main():
    ...
    if mode==0:
        ...
        scaler = StandardScaler()
        intragalactic_data = dropMagn(intragalactic_data)
        ...
        clf = MLPClassifier(max_iter=5, hidden_layer_sizes=(75,75,75))
        ...
        clf = MLPClassifier(hidden_layer_sizes=(85,85,85))
        ...
        extra_model =MLPClassifier(max_iter=5, hidden_layer_sizes=(75,75,75))
        print extragalactic_data.columns
        extra_model.fit(extragalactic_data, extragalactic_targets.values.ravel())
        intra_model = MLPClassifier(max_iter=5, hidden_layer_sizes=(85,85,85))
        ...
        for obj_id, d in get_objects_by_id(filepath):
            ...
            if(count == 10000):
                ...
                if(batch_no==0):
                    finish.to_csv("mlp_feaug.csv", index = False, header = True)
                else:
                    with open("mlp_feaug.csv", 'a') as f:
        ...
        print " >>Clean up."
        preds = pd.read_csv("mlp_feaug.csv")
        preds['object_id']=preds['object_id'].apply(int)
	    #preds['object_id']=preds['object_id'].apply(int)
        print preds.shape
        print cc
        preds.to_csv("mlp_feaug2.csv", index=False)
